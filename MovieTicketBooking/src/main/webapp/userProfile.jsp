...
<ul>
    <%
        ...
        SimpleDateFormat fullDateFormat = new SimpleDateFormat("dd-MM-yyyy HH:mm"); // Full datetime format
        ...
        for (String time : showTimes) {
            try {
                // Parse the selected date and showtime to create a full datetime for comparison
                String fullShowDateTime = selectedDate + " " + time;
                Date showDateTime = fullDateFormat.parse(fullShowDateTime);

                // Format time to 12-hour format
                Date date = inputFormat.parse(time);
                String formattedTime = outputFormat.format(date);

                // Determine if the showtime is in the past
                boolean isDisabled = now.after(showDateTime);
    %>
        <li>
            <a href="seatSelection.jsp?movieID=<%= movieId %>&movieName=<%= movieName%>&theaterName=<%= theaterName %>&location=<%= location %>&theaterID=<%= theaterID %>&ScreenName=<%= ScreenName %>&email=<%= email %>&showTime=<%= formattedTime %>&selectedDate=<%= selectedDate %>"
               ...
               style="<%= isDisabled ? "pointer-events: none; background-color: #ccc; color: #777;" : "" %>">
                <%= formattedTime %>
            </a>
        </li>
    <%
